(() => {
  ...
  const canvas=$('globe'), ctx=canvas.getContext && canvas.getContext('2d'); let rotation=0, dragging=false, previous=0, zoom=1;
  ...
  addEventListener('beforeunload',()=>{clearTimeout(timer);controller?.abort();speechSynthesis?.cancel()});draw();poll();
})();
