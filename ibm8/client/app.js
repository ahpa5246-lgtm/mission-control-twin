(() => {
  ...
  $('reset-view').addEventListener('click',()=>globe3d?.reset());$('follow-iss').addEventListener('click',()=>globe3d?.follow());powerButton.addEventListener('click',e=>{const enabled=e.currentTarget.getAttribute('aria-pressed')!=='true';e.currentTarget.setAttribute('aria-pressed',String(enabled));localStorage.setItem('mission-low-power',String(enabled));e.currentTarget.textContent=enabled?'Low power: on':'Low power';globe3d?.setLowPower(enabled);});
